...
import { ArrowUp, HardDrive, Info, X } from "lucide-react";
...
export const UsageExplorer = ({ repositoryId, snapshotId, schedule, repositorySize, displayBasePath }: Props) => {
	...
	if (!usageData || usageData.status !== "ready") {
					...
					{isComparing && diffQuery.isLoading ? (
						...
						<p className="px-3 py-8 text-center text-sm text-muted-foreground">
							{diffMissing === "against"
								...
								: "Neither snapshot has recorded usage to compare."}
						</p>
					) : (
						<>
							...
							<div className="flex items-start gap-2 border-t border-border px-3 py-3 text-xs text-muted-foreground">
								...
								<div className="space-y-1">
									<p>
										Sizes are the original file sizes on disk, before deduplication and compression
										— the same thing <span className="font-mono">du</span> reports.
									...
									{usageData.meta.source === "backup" && (
										<p>
											Measured from the source while the backup ran, so this includes paths the
											job excludes. The gap against what was actually stored is what your
											exclusions are saving.
										</p>
									)}

...
/**
 * Read a snapshot's directory sizes from the repository with restic
 */
export const scanSnapshotUsageMutation = (options?: Partial<Options<ScanSnapshotUsageData>>): UseMutationOptions<ScanSnapshotUsageResponse, DefaultError, Options<ScanSnapshotUsageData>> => {
    const mutationOptions: UseMutationOptions<ScanSnapshotUsageResponse, DefaultError, Options<ScanSnapshotUsageData>> = {
        mutationFn: async (fnOptions) => {
            const { data } = await scanSnapshotUsage({
                ...options,
                ...fnOptions,
                throwOnError: true
            });
...
export const getSnapshotUsageDiffQueryKey = (options: Options<GetSnapshotUsageDiffData>) => createQueryKey('getSnapshotUsageDiff', options);

/**
 * Compares two snapshots' recorded usage trees to show what grew, shrank, appeared, or disappeared
 */
export const getSnapshotUsageDiffOptions = (options: Options<GetSnapshotUsageDiffData>) => queryOptions<GetSnapshotUsageDiffResponse, DefaultError, GetSnapshotUsageDiffResponse, ReturnType<typeof getSnapshotUsageDiffQueryKey>>({
    queryFn: async ({ queryKey, signal }) => {
        const { data } = await getSnapshotUsageDiff({
            ...options,
            ...queryKey[0],
            signal,
            throwOnError: true
    ...
    queryKey: getSnapshotUsageDiffQueryKey(options)
});
